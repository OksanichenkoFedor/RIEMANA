...
from res.plasma.algo_parts import count_simple_start, count_n_plus, solve_subsistem, count_ions, count_T_e
...
param_vector = (p_0, T_gas, R, L, gamma_cl, y_ar)
...
while (np.abs(delta_T_e) >= 0) and (num <= 15):
    ...
    if num==0:
        params = m_cl
        n_plus = count_n_plus(j, T_e, params, is_m_eff=True, do_print=False)
    else:
        params = (n_plus, n_cl2_plus, n_cl_plus, n_ar_plus)
        n_plus = count_n_plus(j, T_e, params, is_m_eff=False, do_print=True)
    num += 1
