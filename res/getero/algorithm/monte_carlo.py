...
def generate_particles(num, xsize, y_cl, y_ar_plus, y_cl_plus, T_i, T_e):
    ...
    cl_particles = generate_cl(num_cl, xsize, T_i)
    cl_plus_particles = generate_cl_plus(num_cl_plus, xsize, T_i/T_e, T_e)
    ar_plus_particles = generate_ar_plus(num_ar_plus, xsize, T_i/T_e, T_e)
...
def generate_cl_plus(num, xsize, alpha_el, T_e):
    x = np.random.random((num, 1)) * xsize
...
def generate_ar_plus(num, xsize, alpha_el, T_e):
    x = np.random.random((num, 1)) * xsize
    ens = np.ones((num, 1)) * T_e
    sigma = np.arctan(np.sqrt(alpha_el))
    angle = np.random.normal(loc=0, scale=sigma, size=num).reshape((-1, 1))
