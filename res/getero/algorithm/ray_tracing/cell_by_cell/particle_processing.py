...
from res.getero.algorithm.ray_tracing.cell_by_cell.space_orientation import find_next, give_next_cell, throw_particle_away
    ...
    while unfound and not_max_value:
        ...
        if max_value != -1.0:
            if num > max_value:
                not_max_value = False
        num += 1
        #print("find_next: ", empty_prev, prev_att_x, prev_att_y, curr_vec)
        curr_att_x, curr_att_y = find_next(curr_vec[0], curr_vec[1], prev_vec[0], prev_vec[1], prev_att_x, prev_att_y, empty_prev)
        #print(curr_att_x, curr_att_y, curr_vec, is_on_horiz)
        empty_prev = False

        if (curr_att_x == prev_att_x and curr_att_y == prev_att_y) and (not (prev_vec[1] == -10)):
            pass
            print("Ахтунг!!!!")
            print(curr_vec, prev_vec)
        ...
        if is_full_arr[curr_att_x, curr_att_y] == 1.0:
            is_collide_start, cross_vec, curr_segment = check_cell_intersection(border_layer_arr, curr_att_x,
                                                                                curr_att_y,
                                                                                curr_vec, curr_angle)
            is_collide = is_collide_start
            while is_collide:
                ...
                if True:
                    ...
                    #new_type, new_en, flags, redepo_params, new_angle = surface_cycle(counter_arr, is_full_arr,
                    #                                                                  curr_angle, avg_norm_angle,
                    #                                                                  curr_att_x, curr_att_y,
                    #                                                                  prev_att_x, prev_att_y, curr_type,
                    #                                                                  Si_num, curr_en, R)
            ...
            # Маска
            is_collide_start, cross_vec, curr_segment = check_cell_intersection(border_layer_arr, curr_att_x, curr_att_y,
                                                                          curr_vec, curr_angle)
            is_collide = is_collide_start
            while is_collide:
                curr_vec = cross_vec.copy()
            ...
            prev_vec = curr_vec.copy()
            prev_att_x, prev_att_y = curr_att_x, curr_att_y
            curr_vec[0], curr_vec[1], new_is_on_horiz = give_next_cell(prev_vec[0], prev_vec[1], curr_angle, is_on_horiz)
            if test:
                pass
                arr_x.append(curr_vec[0] - 0.5)
                arr_y.append(curr_vec[1] - 0.5)
        ...
        if curr_vec[0] >= xsize:
            #print("fff1: ", curr_vec, prev_vec, curr_att_x, curr_att_y, xsize)
            if do_half:
                prev_att_x, prev_att_y = curr_att_x, curr_att_y
