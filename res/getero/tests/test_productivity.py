...
def count_time(curr_wafer, num_iter, num_per_iter, num_mean=250, test=False, do_half=False):
    ...
    for i in trange(num_iter):
        t1 = time.time()
        params = generate_particles(num_per_iter, curr_wafer.xsize, y_ar_plus=y_ar_plus, y_cl=y_cl,
...
start_wafer = Wafer()
start_wafer.generate_pure_wafer(multiplier, Si_num)
...
start_test_wafer.generate_pure_wafer(multiplier, Si_num)
