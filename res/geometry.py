...
class Cylinder:
    ...
    def generate_points(self,Nz=10,N_phi=10,N_r=10):

        u = np.repeat((np.arange(0, 1, 1.0 / N_phi) * 2 * np.pi).reshape((N_phi, 1)), Nz, axis=1)
        v = np.repeat((np.arange(0, Nz, 1.0) * (self.height / (Nz - 1))).reshape((1, Nz)), N_phi, axis=0)
        ...
        r = np.repeat((np.arange(0, N_r, 1.0) * (self.radius / (N_r - 1))).reshape((1, N_r)), N_phi, axis=0)
        ...
        self.lines = np.array(
            [(self.x * self.radius)+self.start_coord, (self.y * self.radius)+self.start_coord,
             ((-1)*self.x * self.radius)+self.start_coord, ((-1)*self.y * self.radius)+self.start_coord]).reshape(
            (4, 3, 1))
    ...
    def is_points_inside(self,coords):
...
class Conus:
    ...
    def generate_points(self, Nz=10, N_phi=10, N_r_down=10, N_r_up=10):
        u = np.repeat((np.arange(0, 1, 1.0 / N_phi) * 2 * np.pi).reshape((N_phi, 1)), Nz, axis=1)
        v = np.repeat((np.arange(0, Nz, 1.0) * (self.height / (Nz - 1))).reshape((1, Nz)), N_phi, axis=0)
        self.tan = (self.up_radius-self.down_radius)/self.height
        x = self.start_coord[0] + (self.down_radius+v*self.tan) * (self.x[0] * np.cos(u) + self.y[0] * np.sin(u)) + self.z[0] * v
        y = self.start_coord[1] + (self.down_radius+v*self.tan) * (self.x[1] * np.cos(u) + self.y[1] * np.sin(u)) + self.z[1] * v
        z = self.start_coord[2] + (self.down_radius+v*self.tan) * (self.x[2] * np.cos(u) + self.y[2] * np.sin(u)) + self.z[2] * v
        self.bounds = np.concatenate((x.reshape((1, N_phi, Nz)),
                                      y.reshape((1, N_phi, Nz)),
                                      z.reshape((1, N_phi, Nz))), axis=0).reshape((3, -1))

        r = np.repeat((np.arange(0, N_r_down, 1.0) * (self.down_radius / (N_r_down - 1))).reshape((1, N_r_down)), N_phi, axis=0)
        ...
        r = np.repeat((np.arange(0, N_r_up, 1.0) * (self.up_radius / (N_r_up - 1))).reshape((1, N_r_up)), N_phi,
                      axis=0)
        ...
        self.phi = np.arange(0, N_phi * 1.0, 1.0) * 2 * (np.pi / (N_phi - 1.0))
        x = self.start_coord[0] + self.down_radius * (self.x[0] * np.cos(self.phi) + self.y[0] * np.sin(self.phi)) + self.z[
            0] * 0
        y = self.start_coord[1] + self.down_radius * (self.x[1] * np.cos(self.phi) + self.y[1] * np.sin(self.phi)) + self.z[
            1] * 0
        z = self.start_coord[2] + self.down_radius * (self.x[2] * np.cos(self.phi) + self.y[2] * np.sin(self.phi)) + self.z[
            2] * 0
        self.down_arc = np.concatenate((x.reshape((1, N_phi)), y.reshape((1, N_phi)), z.reshape((1, N_phi))), axis=0)
        x = self.start_coord[0] + self.up_radius * (self.x[0] * np.cos(self.phi) + self.y[0] * np.sin(self.phi)) + self.z[
            0] * self.height
        y = self.start_coord[1] + self.up_radius * (self.x[1] * np.cos(self.phi) + self.y[1] * np.sin(self.phi)) + self.z[
            1] * self.height
        z = self.start_coord[2] + self.up_radius * (self.x[2] * np.cos(self.phi) + self.y[2] * np.sin(self.phi)) + self.z[
            2] * self.height
        self.up_arc = np.concatenate((x.reshape((1, N_phi)), y.reshape((1, N_phi)), z.reshape((1, N_phi))), axis=0)
        self.N_phi = N_phi
