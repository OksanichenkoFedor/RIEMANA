...
def initgrid(gridx, gridy, is_inside):
    ...
    grid[0] = (np.random.random((gridx, gridy)) * 2.0 - 1) * V0*0.1
    grid[1] = (np.random.random((gridx, gridy)) * 2.0 - 1) * V0*0.1
...
def step_custom_njit(grid, is_inside, is_inlet, is_outlet, is_x_walls, is_y_walls, is_inl_walls, dx, dy, dt):
    ...
    for x in range(1, newgrid.shape[1] - 1):
        for y in range(1, newgrid.shape[2] - 1):
            u_app = 0.25 * (u[x + 1, y] + u[x - 1, y] + u[x, y - 1] + u[x, y + 1])
            v_app = 0.25 * (v[x + 1, y] + v[x - 1, y] + v[x, y - 1] + v[x, y + 1])
            r_app = rho[x,y]
            ...
            delta_pho_y = rho[x, y + 1] * u[x, y] + u[x, y + 1] * rho[x, y] - \
                          rho[x, y - 1] * u[x, y] - u[x, y - 1] * rho[x, y]  # 1.3.1.4
            ...
            delta_mu_x = (MU / (3.0 * 1.0)) * (
                    4.0 * (dt / (dx * dx)) * delta_u_xx +
                    1.0 * (dt / (4.0 * dx * dy)) * delta_v_xy +
                    3.0 * (dt / (dy * dy)) * delta_u_yy)  # 1.3.2.7
            new_u = u_app + delta_mu_x - \
                    (dt / (2.0*dx)) * u_app * (u[x + 1, y] - u[x - 1, y]) - \
                    (dt / (2.0*dy)) * v_app * (u[x, y + 1] - u[x, y - 1])  # 1.3.2.6
            ...
            delta_mu_y = (MU / (3.0 * 1.0)) * (
                    4.0 * (dt / (dy * dy)) * delta_v_yy +
                    1.0 * (dt / (4.0 * dx * dy)) * delta_u_xy +
                    3.0 * (dt / (dx * dx)) * delta_v_xx)  # 1.3.3.7
            new_v = v_app + delta_mu_y - \
                    (dt / (2*dx)) * u_app * (v[x + 1, y] - v[x - 1, y]) - \
                    (dt / (2*dy)) * v_app * (v[x, y + 1] - v[x, y - 1])  # 1.3.2.6
...
def boundary_step_njit(grid, is_inlet, is_outlet, is_x_walls, is_y_walls, is_inl_walls):
    ...
    for x in range(0, newgrid.shape[1] - 0):
        for y in range(0, newgrid.shape[2] - 0):
            ...
            newgrid[0, x, y] = u[x, y] * (1.0 - is_y_walls[x, y])
            newgrid[1, x, y] = v[x, y] * (1.0 - is_x_walls[x, y] - is_outlet[x, y])
